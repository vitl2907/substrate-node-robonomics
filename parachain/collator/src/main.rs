...
use robonomics::{HeadData as RobonomicsHead, BlockData as RobonomicsBody};
...
const GENESIS: RobonomicsHead = RobonomicsHead {
	number: 0,
	parent_hash: [0; 32],
	post_state: [1, 27, 77, 3, 221, 140, 1, 241, 4, 145, 67, 207, 156, 76, 129, 126, 75, 22, 127, 29, 27, 131, 229, 198, 240, 241, 13, 137, 186, 30, 123, 206],
};

const GENESIS_BODY: RobonomicsBody = RobonomicsBody {
	state: 0,
	add: 0,
};

#[derive(Clone)]
struct RobonomicsContext {
	db: Arc<Mutex<HashMap<RobonomicsHead, RobonomicsBody>>>,
}
...
impl ParachainContext for AdderContext {
	...
	) -> Result<(BlockData, HeadData), InvalidHead>
	{
		...
		let next_head = ::robonomics::execute(adder_head.hash(), adder_head, &next_body)
...
fn main() {
	...
	let res = ::collator::run_collator(
		...
		::std::env::args(),
		VersionInfo {
			name: "robonomics-collator",
			version: "0.0.0",
			commit: "<unknown>",
			executable_name: "robonomics-collator",
			description: "Collator for Robonomics parachain",
			author: "Airalab <research@aira.life>",
			support_url: "https://github.com/airalab/substrate-node-robonomics/issues/new",
		}
	);
